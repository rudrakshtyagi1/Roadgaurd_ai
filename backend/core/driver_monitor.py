...
class DriverMonitor:
    ...
    def __init__(self, face_model_path: Optional[str] = None) -> None:
        ...
        model_file = face_model_path or str(
        ...
        try:
            ...
            if Path(model_file).exists():
                ...
                print(
                    f"[DriverMonitor] ✅ MediaPipe FaceLandmarker "
                    f"task initialized from {model_file}"
                )

            else:
                print(
                    f"[DriverMonitor] ⚠️ MediaPipe task model "
            ...
            print(
                f"[DriverMonitor] ⚠️ MediaPipe unavailable: {e}"
            )
    ...
    def _ear(self, lm: list, indices: list) -> float:
        ...
        num = (
            self._dist(pts[1], pts[5])
            + self._dist(pts[2], pts[4])
        )

        den = (
            2.0
            * self._dist(pts[0], pts[3])
        )
    ...
    def _extract_face_crop(
        ...
        """
        Extract square face crop centered around landmark
        coordinates matching training distribution.
        """
    ...
    def _synthetic(self) -> dict[str, Any]:
        """
        Oscillating synthetic driver state for demo
        when no camera active.
        """
        ...
        return {
            ...
            "blink_count":
                v2_res["metrics"]["total_blink_count"],

            "avg_blink_duration_ms":
                v2_res["metrics"]["avg_blink_duration_ms"],
            ...
            "signal_strength":
                v2_res["signal_strength"],

            "tracking_state":
                v2_res["tracking_state"],
    ...
    def process(
        ...
        Returns unified dictionary with MediaPipe landmarks,
        deep CNN prediction on face crop,
        ...
        try:
            ...
            print(
                f"[DriverMonitor] process() error: {e}"
            )
